...
Treasure_box:: Treasure_box(){
    for(int i = 0; i< ROW; i++){
        for(int j = 0; j < COL; j++){
            this->box[i][j] = 0; //fill zero
        }
    }
}


bool Treasure_box:: is_true(){ //fonction checks if it is treasure
    for (int i = 0; i < ROW; i++){   
        for (int j = 0; j <COL; j++){
...
void Treasure_box:: set_default(){ //fill our matrix by default elements
...
void Treasure_box:: randoma(){//filling random elements
...
bool Treasure_box:: find_special_element(string operation = "work"){ // main function
            ...
            while (true){ //in order to catch the mistake of input
                ...
                try{
                    ...
                    if (cin){ //check if we entered exactly integer
                        if (obj.check_in_array(valuable_integers, elem) ){ //if it's appropiarite integer
